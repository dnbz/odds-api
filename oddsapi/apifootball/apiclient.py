...
from oddsapi.settings import API_URL, API_KEY, API_HOST, HTTPX_PROXY
...
def get_httpx_config(disable_proxy=False) -> dict:
    headers = get_rapidapi_headers()
    limits = httpx.Limits(max_connections=4, max_keepalive_connections=3)

    conf = {"headers": headers, "limits": limits, "timeout": 10.0}

    if not disable_proxy:
        conf["proxies"] = get_httpx_proxy()
